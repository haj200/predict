...
def predict_new(model, vectorizer, mae, model_name):
    while True:
        ...
        print(f"📊 Intervalle estimé : [{lower:.2f} MAD, {upper:.2f} MAD]")

# === 6. Script principal ===
if __name__ == "__main__":
    json_file = "../avis_attribues_nettoyes.json"  # Ton fichier JSON
    ...
    xgb_model, xgb_mae = train_and_evaluate("XGBoost", xgb_model, X_train, X_test, y_train, y_test)
    lgb_model, lgb_mae = train_and_evaluate("LightGBM", lgb_model, X_train, X_test, y_train, y_test)

    # Sauvegarder les modèles et vectoriseur
    print("\n💾 Sauvegarde des modèles et du vectoriseur...")
    joblib.dump(xgb_model, "xgb_model.pkl")
    joblib.dump(lgb_model, "lgb_model.pkl")
    joblib.dump(vectorizer, "tfidf_vectorizer.pkl")
    print("✅ Sauvegarde terminée.")

    # Tester avec un nouvel enregistrement
    print("\n🔍 Teste un nouvel enregistrement avec estimation d'intervalle :")
    predict_new(xgb_model, vectorizer, xgb_mae, "XGBoost")
    predict_new(lgb_model, vectorizer, lgb_mae, "LightGBM")
