...
# === Configuration ===
data_dir = "C:/Users/pc/MarchePub/NewData/data/natures_new"
lemmes_dir = "C:/Users/pc/MarchePub/NewData/data/resultats_par_nature"
output_dir = "C:/Users/pc/MarchePub/NewData/data/prediction/prediction_results/new_data"
...
for filename in os.listdir(data_dir):
    ...
    for item in entries:
        ...
        if best_match:
            ...
            if not correct and montant_real is not None:
                distance_to_interval = min(abs(montant_real - predicted_min), abs(montant_real - predicted_max))
            else:
        ...
        detailed_results.append({
            ...
            "correct": correct,
            "distance_to_interval": distance_to_interval,
            "text": text,
    ...
    with open(stats_path, "w", encoding="utf-8") as f:
        # Calcul du pourcentage d'éléments corrects ou incorrects à <= 30% du montant réel
        within_10pct_by_nature = defaultdict(int)
        total_by_nature = defaultdict(int)
        for res in detailed_results:
            nature = res["nature"]
            montant = res["real_montant"]
            dist = res["distance_to_interval"]
            correct = res["correct"]
            if montant and montant > 0:
                if correct:
                    within_10pct_by_nature[nature] += 1
                elif dist is not None and dist <= 0.1 * montant:
                    within_10pct_by_nature[nature] += 1
                total_by_nature[nature] += 1

        stats_pct = {
            nature: {
                **vals,
                "accuracy": round(vals["correct"] / vals["total"] * 100, 2) if vals["total"] else 0,
                "pct_within_10pct_interval": round(within_10pct_by_nature[nature] / total_by_nature[nature] * 100, 2) if total_by_nature[nature] else 0
            }
